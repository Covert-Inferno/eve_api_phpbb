...
class	Logging
{
				...
				public	function	lwrite(	$message,	$character	)
				{
								$log_file	=	'/var/log/php/phpbb_eve_cron';
								...
								if	(	is_resource($this->fp)	)
								{
												fwrite($this->fp,	"$time ($script_name) [$character] $message"	.	PHP_EOL);
								}
				}
...
function	cronlog(	$message,	$character	=	"---"	)
{
				...
				if	(	is_object($cronlog)	)
				{
								$cronlog->lwrite($message,	$character);
				}
}
...
/**
	...
	*/
function	eveapi_getTeamSpeakUID(	$characterName,	$ts3_VirtualServer	=	false	)
{
				global	$db,	$config;

				if	(	strlen($characterName)	>	30	)
				{
								$characterName	=	substr($characterName,	0,	30);
				}
				...
				try
				{
								...
								$tsClientData	=	$ts3_VirtualServer->clientGetByName($characterName);
								$tsUserUid				=	(string)	$tsClientData->client_unique_identifier;
				}
				catch	(	Exception	$e	)
				{
								return	'';
...
function	eveapi_setOpenFireAccess(	$userId,	$jabber	=	array(),	$extraJabber	=	array(),	$characterName	=	"---",	$password	=	null	)
{
			...
			WHERE user_id = "	.	(int)	$userId;
				...
				if	(	!empty($merge)	)
				{
								...
								{
												$data_jabber	=	array(
																'user_jabber'	=>	$clean_username	.	'@almostawesome.org',
												);
...
function	eveapi_setForumGroups(	$userId,	$forumGroupsNew,	$characterName	=	"---"	)
{
            ...
            WHERE user_id = "	.	(int)	$userId;
				...
				foreach	(	$forumGroups	as	$currentForumGroup	)
				{
								...
								{
												$forumGroupsNew	=	$forumGroupsNew	+	array(
																$currentForumGroup	=>	false);
								}
				}

				// All active users are part of the REGISTERED group, so it is always "New"
				$forumGroupsNew	=	$forumGroupsNew	+	array(
								$registeredGroupID	=>	false);
...
function	eveapi_checkThisCharacter(	$keyID,	$vCode,	$characterName	)
{
				...
				if	(	$characterID	>	0	)
				{
								$result														=	$pheal->eveScope->characterInfo(array(
								...
								if	(	$config['eveapi_fw_masterswitch']	)
								{
												try
												{
																...
																$result														=	$pheal->charScope->FacWarStats(array(
																				"characterID"	=>	$characterID));
...
function	eveapi_getCharacterID(	$characterName	)
{
				...
				try
				{
								$result	=	$pheal->eveScope->CharacterID(array(
												"names"	=>	$characterName));
